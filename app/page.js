...
export default function Home() {
  ...
  useEffect(() => {
    // Sync current language from localStorage
  ...
  // Read arrays directly from the correct language JSON (T.translate can't return arrays)
  const langData = translations[currentLang] || translations["en"];

  // Phone numbers stay constant regardless of language
  const ngoPhones = ["+91-9152987821", "+91-22-23702787", "+91-11-26517295", "+91-44-24640050", "+91-80-25496501"];
  const ngoColors = ["purple", "pink", "rose", "blue", "indigo"];
  const lawyerPhones = ["+91-22-23702787", "+91-11-23385700", "+91-11-24116427", "+91-11-23383714", "15100", "+91-11-23382384"];
  const lawyerColors = ["purple", "pink", "rose", "blue", "green", "indigo"];

  // Read arrays directly from JSON (T.translate returns strings, not arrays)
  const ngoList = (Array.isArray(langData.ngo_list) ? langData.ngo_list : []).map((ngo, i) => ({ ...ngo, phone: ngoPhones[i], color: ngoColors[i] }));
  const lawyerList = (Array.isArray(langData.lawyer_list) ? langData.lawyer_list : []).map((l, i) => ({ ...l, phone: lawyerPhones[i], color: lawyerColors[i] }));

  const helplines = [
    { labelKey: "contact_helpline_women", number: "1091", color: "pink", icon: "M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" },
    { labelKey: "contact_helpline_police", number: "112", color: "red", icon: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" },
    { labelKey: "contact_helpline_ncw", number: "7827170170", color: "purple", icon: "M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-2 5h.01" },
    { labelKey: "contact_helpline_legal", number: "15100", color: "blue", icon: "M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3" },
  ];
          ...
          <div id="ngo-section" className="w-full py-16 px-4 sm:px-6 lg:px-8 bg-transparent">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
                {helplines.map(({ labelKey, number, color, icon }) => (
                  <a key={labelKey} href={`tel:${number}`}
                    ...
                    <p className="text-xs text-gray-500 font-medium mb-1">{T.translate(labelKey) || labelKey}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white/30 border border-gray-100/40 rounded-2xl p-6">
                  <div className="flex gap-2 mb-6">
                    <button onClick={() => setContactTab("ngo")}
                      ...
                      {T.translate("contact_tab_ngo") || "NGOs"}
                    </button>
                    <button onClick={() => setContactTab("lawyers")}
                      ...
                      {T.translate("contact_tab_lawyers") || "Lawyers"}
                    </button>
                  ...
                  {contactTab === "ngo" && (
                    <div className="space-y-3">
                      {ngoList.map(({ name, phone, type, city, color }) => (
                        <div key={name} className="flex items-center justify-between p-3.5 bg-white/40 rounded-xl border border-gray-100/30 hover:bg-white/70 transition-all">
                          ...
                          <a href={`tel:${phone.replace(/[\s-]/g, "")}`}
                            ...
                            {T.translate("contact_call_btn") || "Call"}
                          </a>
                        </div>
                      ))}
                    </div>
                  ...
                  {contactTab === "lawyers" && (
                    <div className="space-y-3">
                      {lawyerList.map(({ name, phone, spec, city, color }) => (
                        <div key={name} className="flex items-center justify-between p-3.5 bg-white/40 rounded-xl border border-gray-100/30 hover:bg-white/70 transition-all">
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 bg-${color}-100 rounded-full flex items-center justify-center flex-shrink-0`}>
                ...
                <div className="flex flex-col gap-4">
                  <div className="bg-gradient-to-br from-purple-600 to-pink-500 rounded-2xl p-6 text-white flex flex-col shadow-lg">
                    ...
                    <h3 className="text-xl font-bold mb-2">{T.translate("contact_chat_title") || "Chat with a Lawyer"}</h3>
                    <p className="text-white/80 text-sm mb-6">{T.translate("contact_chat_desc") || "Get instant legal guidance from our AI Legal Advisor."}</p>
                    <Link href="/chatbot" className="bg-white text-purple-700 hover:bg-purple-50 font-bold text-center py-3 px-6 rounded-full transition-all hover:shadow-lg text-sm">
                      {T.translate("contact_chat_btn") || "Start Free Consultation →"}
                    </Link>
                    <div className="mt-4 pt-4 border-t border-white/20">
                      <p className="text-white/70 text-xs text-center">🔒 {T.translate("contact_chat_secure") || "Confidential & Secure"}</p>
                  ...
                  <Link href="/complaint" className="bg-white/30 border border-blue-100/40 hover:bg-white/60 rounded-2xl p-5 flex items-center gap-4 hover:-translate-y-0.5 hover:shadow-lg transition-all group">
                    ...
                    <div>
                      <p className="font-bold text-gray-800 text-sm">{T.translate("contact_file_title") || "File a Complaint"}</p>
                      <p className="text-xs text-gray-500">{T.translate("contact_file_desc") || "Document your case securely"}</p>
